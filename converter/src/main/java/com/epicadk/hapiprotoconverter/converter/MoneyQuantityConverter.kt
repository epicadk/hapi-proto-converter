...
public object MoneyQuantityConverter {
  ...
  public fun org.hl7.fhir.r4.model.MoneyQuantity.toProto(): MoneyQuantity {
    ...
    if (hasComparator()) {
      protoValue.setComparator(MoneyQuantity.ComparatorCode.newBuilder().setValue(QuantityComparatorCode.Value.valueOf(comparator.toCode().protoCodeCheck().replace("-",
          "_").toUpperCase())).build())
    }
    if (hasUnit()) {
