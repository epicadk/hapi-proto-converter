...
public object SlotConverter {
  public fun Slot.toHapi(): org.hl7.fhir.r4.model.Slot {
    ...
    if (hasStatus()) {
      hapiValue.setStatus(org.hl7.fhir.r4.model.Slot.SlotStatus.valueOf(status.value.name.hapiCodeCheck().replace("_", "")))
    }
    if (hasStart()) {
  ...
  public fun org.hl7.fhir.r4.model.Slot.toProto(): Slot {
    ...
    if (hasStatus()) {
      protoValue.setStatus(Slot.StatusCode.newBuilder().setValue(SlotStatusCode.Value.valueOf(status.toCode().protoCodeCheck().replace("-",
          "_").toUpperCase())).build())
    }
    if (hasStart()) {
